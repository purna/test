...
class GoogleSheetsManager {
    ...
    setupGoogleSignIn() {
        ...
        if (isConfigured) {
            ...
            // Show the GSI button if it was hidden earlier
            if (gIdOnload) gIdOnload.parentElement.style.display = '';
            if (gSignInBtn) gSignInBtn.style.display = '';
        } else {
        ...
        // Wire up any dynamically generated GSI buttons
        this.setupGSIButtonHandlers();
    }

    setupGSIButtonHandlers() {
        // The GSI button container is dynamically generated by Google's library
        // We need to re-inject it with proper handlers after our config check
        const gIdOnload = document.getElementById('g_id_onload');
        const gSignInBtnContainer = gIdOnload?.parentElement;
        
        if (gIdOnload && gSignInBtnContainer) {
            // Ensure the container is visible if configured
            const isConfigured = this.clientId && 
                this.clientId !== 'YOUR_CLIENT_ID.apps.googleusercontent.com' &&
                !this.clientId.includes('YOUR_');
            if (isConfigured) {
                gSignInBtnContainer.style.display = '';
            }
        }
    }
